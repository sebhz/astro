...
class Sundial:
    ...
    def __init__(self, phi, D, z, a, l=0.0):
        ...
        try:
            self.center = (a*cos(phi_r)*sin(D_r)/self.P,\
                           -a*(sin(phi_r)*sin(z_r) + cos(phi_r)*cos(z_r)*cos(D_r))/self.P, )
    ...
    def compute_hour_lines(self):
        ...
        for H in range(-12, 12):
            ...
            for delta in Sundial.declinations:
                ...
                Q = sin(D_r)*sin(z_r)*sin(H_r) + \
                    (cos(phi_r)*cos(z_r)+sin(phi_r)*sin(z_r)*cos(D_r))*cos(H_r) + \
                    self.P*tan(delta_r)
                if Q < 0: # Sun does not illuminate the plane for this declination
                    continue
                Nx = cos(D_r)*sin(H_r) - sin(D_r)*(sin(phi_r)*cos(H_r)-cos(phi_r)*tan(delta_r))
                Ny = cos(z_r)*sin(D_r)*sin(H_r) - \
                     (cos(phi_r)*sin(z_r) - sin(phi_r)*cos(z_r)*cos(D_r))*cos(H_r) - \
                     (sin(phi_r)*sin(z_r) + cos(phi_r)*cos(z_r)*cos(D_r))*tan(delta_r)
    ...
    def get_svg(self):
        ...
        for furthest_point in lines_h:
            hours_t.append((furthest_point[0], \
...
PARSER.add_argument("--phi", help='Latitude of the sundial in degrees. Positive towards north', \
                             type=float, required=True)
PARSER.add_argument("--D", \
                    help='Declination of the sundial plane perpendicular in degrees', \
                    type=float, required=True)
PARSER.add_argument("--z", help='Zenithal distance of the stylus in degrees', type=float, \
                    required=True)
PARSER.add_argument("--a", help='Length of the stylus (default: {:0.4f})'.format(DEFAULT_A), \
                    type=float, default=DEFAULT_A)
PARSER.add_argument("--l", help='Longitude of the sundial in degrees.', type=float, default=0.0)
PARSER.add_argument("--svg", help='Displays SVG rather than plain text', action='store_true', \
                    default=False)
